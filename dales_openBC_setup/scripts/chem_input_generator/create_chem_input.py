# Script to create DALES chemical boundaries from LOTOS-EUROS/CAMS/IFS output
...
class Setup:
    def __init__(self, input_file='../input_coarse.json'):
        ...
        # --- Open dataset (once!) ---
        ds = xr.open_dataset(f"{self.input_dir}{self.input_file}")
        ...
        print(f"Approx. LOTOS-EUROS/CAMS/IFS horisontal resolution in meters: dx={dx:.1f} m, dy={dy:.1f} m")
        print(f"Using Gaussian sigmas: σx={self.sigma_x_original:.2f} km, σy={self.sigma_y_original:.2f} km")

        # --- Set up tracer names ---
        self.tracer_names = config.tracer_names
...
def write_initfield_nc(dataset, tracer_name, target_grid, hour, minutes, iexpnr,
                       output_dir, transform, sigma_x, sigma_y, lons, lats ):
    """
    ...
    if not os.path.exists(name):
        ...
        #lats and lons from xt,yt --> yt,xt:
        lats = lats.T
        lons = lons.T
        ...
        with nc4.Dataset(name, "w") as f:
            ...
            # --- LES latitude and longitude ---
            lat = f.createVariable("lat", "f4", ("yt", "xt"))
            lon = f.createVariable("lon", "f4", ("yt", "xt"))
...
if __name__ == "__main__":
    ...
    file_path = f'{setup.input_dir}{setup.input_file}'
    ...
    if setup.input_type == 'LE':
        ...
        if altitude[0, :, :].mean() > altitude[-1, :, :].mean():
            altitude = altitude[::-1, :, :].copy()
            inverse=True
            print('WARNING: altitudes are inversed (top->bottom): flipping to bottom->up is applied! ')
        ...
        for i, t in enumerate(range(t0, t1+1)):  # i tracks the timestep index  t1+1
            ...
            try: 
                ...
                if t == t0:
                    ...
                    write_initfield_nc(LES_interpolated, tracer_name, setup.target_grid, t - t0, 0., setup.iexpnr, setup.output_dir, transform, sigma_x, sigma_y, les_lons, les_lats ) 
