...
def initial_fields(input,grid,data,transform):
  ...
  qr  = data['qr'].interp(z=grid.zt, y=grid.yt, x=grid.xt, assume_sorted=True).rename({'z': 'zt', 'y': 'yt', "x": 'xt'}).rename('qr')
  ...
  initfields = xr.merge([u0,v0,w0,thl0,qt0,e120,qr,lat,lon],combine_attrs='drop')
  # Add transform
  ...
  initfields['qr'] = initfields['qr'].assign_attrs({'longname': 'Initial rain water mixing ratio','units': 'kg/kg'})
