...
def prep_harmonie(input,grid):
  ...
  for var in variables:
    print(f"trying to open {input['inpath']}{var}_*_LCC.nc")
    with xr.open_mfdataset(f"{input['inpath']}{var}_*_LCC.nc",chunks={"time": input['tchunk']}) as ds:
      # Get time epochs
      if(var==variables[0]):
  ...
  coeff = np.loadtxt(f"H43_65lev.txt")
