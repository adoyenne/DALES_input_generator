...
def prep_harmonie(input,grid):
  variables = ['ua','va','wa','ta','hus','clw','qr','ps','tas','huss']
  if('synturb' in input): variables = variables+['tke','tauu','tauv','cb','hfss']
  data = []
  # Open data and crop data
  for var in variables:
    print(f"trying to open {input['inpath']}{var}_*_RD.nc")
    with xr.open_mfdataset(f"{input['inpath']}{var}_*_RD.nc",chunks={"time": input['tchunk']}) as ds:
      # Get time epochs
      if(var==variables[0]):
        # Read transform information and transform lat/lon of southwest corner to harmonie x/y
        transform = Transform(ds['RD_coordinates'].attrs)
  ...
  coeff = np.loadtxt(input['hybrid_file'])
  ...
  variables = ['uas','vas','was','clws', 'qrs']
  ...
  variables = ['ua','va','wa','ta','hus','clw','p','qr']
  ...
  # Calculate domain-mean T,P, and Q for backrad calculation:
  
  p = 0.5*(ph.assign_coords({'lev': ph['lev'].values-1})+ph)
  tah=0.5*(data['ta'].assign_coords({'lev': data['ta']['lev'].values-1})+data['ta'])
  hush=0.5*(data['hus'].assign_coords({'lev': data['hus']['lev'].values-1})+data['hus'])
  
  T_mean  = tah[:,::-1,:,:].values.mean(axis=(0,2,3))
  qv_mean = hush[:,::-1,:,:].values.mean(axis=(0,2,3))
  p_mean = p[:,::-1,:,:].values.mean(axis=(0,2,3))
  
  nc_file = nc.Dataset(f"{input['outpath']}/backrad.inp.{input['iexpnr']:03d}.nc", 'w')
  ...
  new_coords = {'time': data.coords['time'],
                ...
                'x': data.coords['x']}
  new_dims = ['time','z','y','x']
  variables = ['ua','va','wa','ta','p','hus','clw','qr']
  if('synturb' in input): variables.append('tke')
  for var in variables:
    its = 0
    var_intz = []
  ...
  if(input['start']==input['time0']): # Calculate exnr function
    ...
    qt_exnr  = data['qt'].isel({'time': 0, 'z': 0},drop=True).sel(x=slice(0,grid.xsize),y=slice(0,grid.ysize)).mean(dim=['x','y']).values
    ...
    qt_exnr = float(line0.split(',')[3].split('qt = ')[-1])
    exnr = np.loadtxt(input['exnr_file'],skiprows=1)
    exnrs = exnr[0,1]
    exnr = exnr[1:,1]
  print(f'ps = {ps_exnr}')
  ...
  data = data.rename({'ua': 'u', 'va': 'v', 'wa': 'w'})\
    .drop(['ta','p','clw','hus'])\
    .assign({'transform' : xr.DataArray([],name='RD_coordinates',attrs=transform.parameters)})
